...
// .wrangler/tmp/bundle-cuOTZd/checked-fetch.js
...
// .wrangler/tmp/bundle-cuOTZd/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-cuOTZd/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var f = class {
  ...
  async request(e) {
    let t = { cache: this.options.cache, method: "POST", headers: this.headers, body: JSON.stringify(e.body), keepalive: true, signal: this.options.signal }, a = null, o = null;
    for (let g = 0; g <= this.retry.attempts; g++)
      try {
        a = await fetch([this.baseUrl, ...e.path ?? []].join("/"), t);
        break;
      } catch (b) {
        if (this.options.signal?.aborted) {
          let M = new Blob([JSON.stringify({ result: this.options.signal.reason ?? "Aborted" })]), A = { status: 200, statusText: this.options.signal.reason ?? "Aborted" };
          a = new Response(M, A);
          break;
        }
        o = b, await new Promise((M) => setTimeout(M, this.retry.backoff(g)));
      }
    if (!a)
    ...
    if (!a.ok)
      throw new i(`${s.error}`);
    return { result: s.result, error: s.error };
  }
};
var n = class {
  payload;
  ...
  async exec(e) {
    let { result: t, error: a } = await e.request({ body: this.payload, path: [this.endpoint] });
    if (a)
      throw new i(a);
    if (typeof t > "u")
...
var p = class extends n {
  constructor(e, t) {
    let a = "delete";
    t?.namespace && (a = `${a}/${t.namespace}`);
    let o = [];
    Array.isArray(e) ? o.push(...e) : o.push(e), super(o, a);
  }
};
var d = class extends n {
  constructor(e, t) {
    let a = "query";
    "data" in e && (a = "query-data"), t?.namespace && (a = `${a}/${t.namespace}`), super(e, a);
  }
};
var m = class extends n {
  constructor(e, t) {
    let a = "upsert";
    if (Array.isArray(e)) {
      if (e.some((s) => "data" in s && s.data)) {
        a = "upsert-data";
        for (let s of e)
          !("metadata" in s) && "data" in s && (s.metadata = { data: s.data });
      }
    } else
      "data" in e && (a = "upsert-data", "metadata" in e || (e.metadata = { data: e.data }));
    t?.namespace && (a = `${a}/${t.namespace}`), super(e, a);
  }
};
var c = class extends n {
  constructor([e, t]) {
    let a = "fetch";
    t?.namespace && (a = `${a}/${t.namespace}`, delete t.namespace), super({ ids: e, ...t }, a);
  }
};
...
var u = class extends n {
  constructor(e) {
    let t = "reset";
    e?.namespace && (t = `${t}/${e.namespace}`), super([], t);
  }
};
...
var h = class {
  ...
  upsert = (e) => new m(e, { namespace: this.namespace }).exec(this.client);
  update = (e) => new y(e, { namespace: this.namespace }).exec(this.client);
  fetch = (...e) => (e[1] ? e[1].namespace = this.namespace : e[1] = { namespace: this.namespace }, new c(e).exec(this.client));
  query = (e) => new d(e, { namespace: this.namespace }).exec(this.client);
  delete = (e) => new p(e, { namespace: this.namespace }).exec(this.client);
  range = (e) => new l(e, { namespace: this.namespace }).exec(this.client);
  reset = () => new u({ namespace: this.namespace }).exec(this.client);
};
...
var E = class {
  ...
  namespace = (e) => new h(this.client, e);
  delete = (e, t) => new p(e, t).exec(this.client);
  query = (e, t) => new d(e, t).exec(this.client);
  upsert = (e, t) => new m(e, t).exec(this.client);
  update = (e, t) => new y(e, t).exec(this.client);
  fetch = (...e) => new c(e).exec(this.client);
  reset = (e) => new u(e).exec(this.client);
  range = (e, t) => new l(e, t).exec(this.client);
  info = () => new T().exec(this.client);
  listNamespaces = () => new C().exec(this.client);
  deleteNamespace = (e) => new x(e).exec(this.client);
};
var R = class r extends E {
  constructor(e) {
    ...
    let t = e?.token ?? process.env.NEXT_PUBLIC_UPSTASH_VECTOR_REST_TOKEN ?? process.env.UPSTASH_VECTOR_REST_TOKEN, a = e?.url ?? process.env.NEXT_PUBLIC_UPSTASH_VECTOR_REST_URL ?? process.env.UPSTASH_VECTOR_REST_URL;
    if (!t)
      throw new Error("UPSTASH_VECTOR_REST_TOKEN is missing!");
    if (!a)
      throw new Error("UPSTASH_VECTOR_REST_URL is missing!");
    (a.startsWith(" ") || a.endsWith(" ") || /\r|\n/.test(a)) && console.warn("The vector url contains whitespace or newline, which can cause errors!"), (t.startsWith(" ") || t.endsWith(" ") || /\r|\n/.test(t)) && console.warn("The vector token contains whitespace or newline, which can cause errors!");
    let o = new f({ baseUrl: a, retry: e?.retry, headers: { authorization: `Bearer ${t}` }, cache: e?.cache === false ? void 0 : e?.cache || "no-store", signal: e?.signal });
    super(o);
  }
  static fromEnv(e) {
...
var isIterator = (x2) => x2 != null && typeof x2 === "object" && "next" in x2 && typeof x2.next === "function";
...
app.post("/api", async (c2) => {
  ...
  try {
    ...
    if (message.length > 1e3) {
      return c2.json({ error: "Message is too long, it can atmost be 1000 characters" }, { status: 413 });
    }
    ...
    console.log(error);
    return c2.json({ error: "Internal Server Error" }, { status: 500 });
  }
});
...
var hono_upstash_cloudflare_profanity_api_default = app;
...
var middleware_insertion_facade_default = hono_upstash_cloudflare_profanity_api_default;
